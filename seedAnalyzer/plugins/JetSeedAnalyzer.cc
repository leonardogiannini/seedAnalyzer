...
class JetSeedAnalyzer : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   ...
   private:
      ...
      int min3DIPValue=0.005;
      int min3DIPSignificance=1.2;
...
//            }
            ...
            if ((b_found || d_found)&&bd_i)
            {
                ...
                std::cout << " bfound  "<< b_found << " dfound  "<< d_found << " same chain  "<< same_chain << "  bdi  "<< bd_i <<std::endl;
            }
...
//     int d_found=0;
    ...
    {
         std::cout<< "motherspdid  " << mothers[0]->pdgId()<<std::endl;
    id_mom = abs(mothers[0]->pdgId());
    id_mom=std::max((id_mom/1000) % 10,(id_mom/100) % 10);
    if (id_mom==5){
//     b_found=1;
